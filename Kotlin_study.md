...
## 배열
ex) ```
var intArr = arrayOf(1,2,3,4,5)``` 배열에 들어갈 값 저장<br/>
ex) ```var nullArr = arrayOfNulls<Int>(5)``` 크기가 5이고 null값으로 채워진 배열 생성
<>안에 배열에 할당할 자료형 지정
...
ex) for(i in 0..9 step 3){} 0 -> 3 -> 6 -> 9
...
```
- 추상클래스는 미완성 클래스이기 때문에 단독으로는 인스턴스를 만들 수 없음, 따라서반드시 서브클래스에서 상속을 받아 abstract표시가 된 함수들을 구현해줘야함<br/>
...
- companion object : 클래스의 인스턴스 기능은 그대로 사용하면서 인스턴스간에 공용으로 사용할 속성 및 함수를 별도로 만드는 기능
ex) ```class FoodPoll (val name: String){
	companion object{
		var total = 0
	}
	var count = 0
...
- 비동기 : 요청과 결과가 동시에 일어나지 않음, 결과가 주어지는데 시간이 걸리더라도 그 시간 동안에 다른 작업을 할 수 있음, 효율적
